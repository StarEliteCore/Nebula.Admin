...
namespace Destiny.Core.Flow.IdentityServer.Validation
{
    public class TokenRequestBaseValidator : ITokenRequestValidator
    {
        ...
        public TokenRequestBaseValidator(IdentityServerOptions options,
            ...
            ISystemClock clock,
            ILogger<TokenRequestBaseValidator> logger)
        {
            _logger = logger;
            _options = options;
            _clock = clock;
        ...
        private async Task<TokenRequestValidationResult> RunValidationAsync(Func<NameValueCollection, Task<TokenRequestValidationResult>> validationFunc, NameValueCollection parameters)
        {
            ...
            {
                if (customValidationContext.Result.Error.IsNullOrEmpty())
                {
        ...
        private async Task<TokenRequestValidationResult> ValidateAuthorizationCodeRequestAsync(NameValueCollection parameters)
        {
            ...
            // populate session id
            /////////////////////////////////////////////
            if (authZcode.SessionId.IsNullOrEmpty())
            {
            ...
            if (_validatedRequest.Client.RequirePkce || _validatedRequest.AuthorizationCode.CodeChallenge.IsNullOrEmpty())
            {
        ...
        private async Task<TokenRequestValidationResult> ValidateResourceOwnerCredentialRequestAsync(NameValueCollection parameters)
        {
            ...
            if (resourceOwnerContext.Result.IsError)
            {
                ...
                if (resourceOwnerContext.Result.ErrorDescription.IsNullOrEmpty())
                {
        ...
        private async Task<TokenRequestValidationResult> ValidateRefreshTokenRequestAsync(NameValueCollection parameters)
        {
            ...
            if (refreshTokenHandle.IsNullOrEmpty())
            {
        ...
        {
            if (authZcode.CodeChallenge.IsNullOrEmpty() || authZcode.CodeChallengeMethod.IsNullOrEmpty())
            {
