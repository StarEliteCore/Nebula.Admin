...
namespace Destiny.Core.Flow.OpenIddict.EntityFrameworkCore.Store
{
    ...
    //    OpenIddictEntityFrameworkCoreAuthorizationStore<Authorization,
    //                                                    Application,
    //                                                    Token, TContext, Guid>
    ...
    public class OpenIddictEntityFrameworkCoreAuthorizationStore<TContext, TKey> :
        OpenIddictEntityFrameworkCoreAuthorizationStore<AuthorizationBase<TKey>,
                                                        ApplicationBase<TKey>,
                                                        TokenBase<TKey>, TContext, TKey>
        where TContext : DbContext
    ...
    public class OpenIddictEntityFrameworkCoreAuthorizationStore<TAuthorization, TApplication, TToken, TContext, TKey> : IOpenIddictAuthorizationStore<TAuthorization>
        where TAuthorization : AuthorizationBase<TKey, TApplication, TAuthorization>
        where TToken : TokenBase<TKey, TApplication, TAuthorization>
        where TApplication : ApplicationBase<TKey, TAuthorization, TToken>
        where TContext : DbContext
        where TKey : IEquatable<TKey>
    {
        public OpenIddictEntityFrameworkCoreAuthorizationStore(
        ...
        public virtual ValueTask<DateTimeOffset?> GetCreationDateAsync(TAuthorization authorization, CancellationToken cancellationToken)
        {
            ...
            return new ValueTask<DateTimeOffset?>(DateTime.SpecifyKind(authorization.CreatedTime, DateTimeKind.Utc));
        }
            ...
            for (var index = 0; index < 1_000; index++)
            {
                ...
                var authorizations =
                    await (from authorization in Authorizations.Include(authorization => authorization.Tokens).AsTracking()
                           where authorization.CreatedTime < date
            ...
            DateTimeOffset? date, CancellationToken cancellationToken)
        {
            ...
            authorization.CreatedTime = date?.UtcDateTime??default;
